...
### テクスチャを読み込んだ後の colorSpace の設定の意味
...
- テクスチャーのカラースペースを指定しないと、実際のテクスチャー画像よりも明るい画像が表示される問題が発生することがある
    ...
    →[原因](#threejs-にてテクスチャーのカラースペースを指定しなかった時にテクスチャーの色が実際のものより明るく表示されてしまった理由)
...
3. 処理結果をリニアー → 元のカラー空間(+ ガンマ補正)に変換する
...
### Three.js にてテクスチャーのカラースペースを指定しなかった時にテクスチャーの色が実際のものより明るく表示されてしまった理由
