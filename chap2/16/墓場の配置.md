...
- 墓場オブジェクトのxのポジションをsin, 残りのポジション(z)をcosで設定すればいい
    ...
    */
    ...
    grave.position.z = Math.cos(theta);

    //↑はこのコードでもいい
    grave.position.x = Math.cos(theta);
    grave.position.z = Math.sin(theta);
    ```

    <br>

- 下の緑の円がオブジェクトの取りうるポジション
